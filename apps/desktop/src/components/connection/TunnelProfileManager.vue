...
watch(
  ...
  ([loaded]) => {
    if (!loaded) return;
    // Seed the draft the first time the store loads (including when this panel
    // remounts against an already-loaded store — draft starts empty then, so
    // isDirty is true and must not gate the initial seed). Afterwards a
    // background refresh only re-seeds when there are no unsaved edits.
